...
- Name: smoke.spec.ts >> Export >> export UI trigger [UI gap — no export button in current SPA]
- Location: e2e/smoke.spec.ts:629:3
...
Locator: getByRole('button', { name: /export/i }).or(getByRole('link', { name: /export/i }))
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /export/i }).or(getByRole('link', { name: /export/i }))
