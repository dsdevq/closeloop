...
_COMPARE_OPS = {"eq", "neq", "gt", "gte", "lt", "lte", "contains", "starts_with", "in", "has_any"}
...
def _compare(field_val: Any, op: str, value: Any) -> bool:
    ...
    if op == "in":
        # field_val (scalar) must be a member of value (list)
        if not isinstance(value, list):
            raise ValueError(f"'in' op requires value to be a list, got {type(value)!r}")
        return field_val in value
    if op == "has_any":
        # field_val (list) must share at least one element with value (list)
        if not isinstance(value, list):
            raise ValueError(f"'has_any' op requires value to be a list, got {type(value)!r}")
        if not isinstance(field_val, list):
            # treat scalar field as single-element list
            field_val = [field_val]
        return bool(set(field_val) & set(value))
