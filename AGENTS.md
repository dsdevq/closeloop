...
**Done:** Workflow automation slice 1 — rule definition model + trigger wiring. `app/core/automations.py` provides typed `Condition` dataclass, `evaluate_conditions(entity_snapshot, conditions)` (pure, AND-only conjunctive), `render_message_template()`, `SUPPORTED_OPS`, `SUPPORTED_TRIGGER_KINDS` (mirrors `_KIND_MAP` in `app/core/history.py` — enforced by `tests/test_core_automations.py::test_supported_trigger_kinds_match_history_kind_map`), `SUPPORTED_ENTITY_TYPES`. `AutomationRule` ORM model in `app/models.py` (`automation_rules` table): columns `name`, `entity_type`, `trigger_kind`, `conditions_json`, `action_kind`, `action_params_json`, `is_active`, `created_by_id`; composite index on `(entity_type, trigger_kind, is_active)` — supports the WHERE query at each trigger site. `app/services/automations.py` provides `execute_automation_rules(db, *, trigger_kind, entity_type, entity_snapshot, actor, clk)` — query + evaluate + execute loop (does NOT commit; caller owns transaction). `execute_automation_action()` dispatches to `_action_notify_user()` which calls the existing `create_notification()` — automation is a new caller of an existing service, not a new notification path (Attio pattern). Self-notification suppression (`recipient_id == actor.id`) reuses the same guard. Action produces `AutomationTriggeredEvent` (new kind in `app/core/notifications.py`; `render_notification()` returns `event.message`). **Where rules are registered:** `AutomationRule` rows are created directly in the DB (slice 1 has no CRUD REST API — that is slice 2). **Where rules are evaluated:** `execute_automation_rules()` is called inline in route handlers at:
- `app/routers/deals.py`: `create_deal` (kind=`deal_created`); `update_deal_stage` when stage changes (kind=`deal_stage_changed`); `update_deal` for `deal_stage_changed` / `deal_assigned` / `deal_updated` depending on what changed.
- `app/routers/contacts.py`: `create_contact` (kind=`contact_created`); `update_contact` when payload non-empty (kind=`contact_updated`).
- `app/routers/activities.py`: `create_activity` (kind=`activity_created`); `complete_activity` (kind=`activity_completed`).
All calls are placed after `record_history()` and before `db.commit()`, in the same After-Save transaction. Tests in `tests/test_core_automations.py` (pure) and `tests/test_automation_triggers.py` (API integration). CRUD API for rules (slice 2), `create_activity` action (slice 2), field-level from/to conditions (slice 3), and execution log (slice 4) are deferred. Domain concepts documented in `DOMAIN.md`. Trigger/composition model borrowed from Salesforce Flow (After-Save timing, closed trigger enum), HubSpot Workflows (single-entity-type scope, declarative enrollment criteria), Zoho Workflow Rules (trigger→criteria→action shape), Attio (inline mutation-site execution), Pipedrive (closed trigger-event enum) — see `.devclaw/research/workflow-automation.md` for full survey and rejected alternatives.
