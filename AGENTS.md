...
**Done:** Notifications engine slice 4 — in-app notification centre (HubSpot-style bell + dropdown). `frontend/src/features/notifications/NotificationCenter.tsx` is a self-contained React component (no App-level state): mounts in `AppHeader`, contains its own `useNotifications` hook (`frontend/src/hooks/useNotifications.ts`) that polls `GET /notifications/unread-count` every 30 s, loads the full list from `GET /notifications?limit=50` when the panel opens, and calls `POST /notifications/{id}/read` / `POST /notifications/read-all` for read-state mutations. Design rationale from `notifications-engine.md` §2–3: HubSpot pull-model pattern (not Chatter/feed page), Pipedrive unread-count badge + closed-kind icon dispatch, HubSpot/Attio `read_at` timestamp (not boolean), Attio flat list (Zoho day-grouping rejected as a frontend concern). `AppHeader` now accepts `isAuthenticated: boolean` prop (passed from `App.tsx`) and renders `NotificationCenter` between the user pill and the logout button. `Notification` and `NotificationKind` types added to `frontend/src/types.ts`. E2e coverage in `e2e/notifications.spec.ts`: bell visible, badge on unread, panel open/close (Escape + click-outside), empty state, click-to-mark-read, mark-all-read — all wired to real backend via a second (rep) user triggering `MentionEvent` for admin via `POST /activities` with `@admin` in the note body. `data-testid` attributes (`notification-bell`, `notification-badge`, `notification-panel`, `notification-empty`, `mark-all-read-btn`) are the stable selectors for e2e.

**Done:** Automation engine slice 3 — `notify` action handler. `_execute_action` in `app/services/automations.py` is no longer a stub: the `"notify"` action type now calls `_execute_notify_action`, which creates an `AutomationEvent` `Notification` row via `create_notification()` (same service entry point as after-save triggers in routers). Key details: `ActionConfigParseError` raised by `_parse_notify_config` skips the action (fail-closed, same contract as `ConditionsParseError`). Recipient resolved from `action_config_json` via `recipient_id` (static) or `recipient_field` (name of a context key holding the user ID). Self-notifications suppressed (actor_id == recipient_id). `entity_type`/`entity_id` forwarded from context to the Notification row. Scheduled rules have `actor_id=None` (no human actor). Unknown action types are logged and skipped — forward-compatible with future action types. `AutomationEvent` dataclass in `app/core/notifications.py` carries `rule_id`/`rule_name`/`actor_id`; rendered at read time by `render_notification()` (rejected pattern: pre-rendered string → stale text on rule rename). Tests in `tests/test_automation_notification_action.py` (36 tests). Reference CRMs: HubSpot (server-side automation notification, category-keyed), Salesforce (typed payload Custom Notification), Attio (actor_id first-class), Pipedrive (entity_type + entity_id for frontend navigation). See `DOMAIN.md §v4 AutomationAction`.
