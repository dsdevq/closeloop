...
def run_scheduled_automations(db: Session, *, clk: Clock) -> int:
    ...
    For each scheduled rule:
      ...
      4. Parse conditions_json (fail-closed on ConditionsParseError).
      5. Evaluate conditions against an empty context dict (scheduled rules
      ...
      6. Call _execute_action(), then record last_triggered_at = now.
