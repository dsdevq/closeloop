...
Multi-worker race condition prevention:
  ...
  worker's UPDATE wins.  See also: DOMAIN.md §ScheduledTrigger.
...
def run_scheduled_automations(db: Session, *, clk: Clock) -> int:
    ...
    this function is the transaction owner: it commits after firing due rules.
    It is called exclusively by the background poller in app/main.py, never
    inline in a request handler — no second evaluation pipeline is introduced.
    ...
    For each scheduled rule:
      ...
      5. Parse conditions_json (fail-closed on ConditionsParseError).
      6. Evaluate conditions against an empty context dict (scheduled rules
